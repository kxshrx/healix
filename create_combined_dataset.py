...
def main():
    ...
    final_columns = [
        ...
        'claim_id', 'patient_hash', 'age', 'gender', 'blood_type', 
        'medical_condition', 'admission_year_month', 'admission_type',
        'length_of_stay_days', 'discharge_date', 'medication', 
        'test_results', 'insurance_provider', 'billing_amount', 'created_at',
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_combined_provider ON claims_with_policy_rules(insurance_provider)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_combined_policy_id ON claims_with_policy_rules(policy_id)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_combined_plan_type ON claims_with_policy_rules(plan_type)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_combined_medical_condition ON claims_with_policy_rules(medical_condition)")
    ...
    try:
        ...
        # Get column info
        column_info = pd.read_sql_query("PRAGMA table_info(claims_with_policy_rules)", conn)
        print("\nColumn Details:")
